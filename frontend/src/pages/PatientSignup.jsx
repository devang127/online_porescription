...
const PatientSignup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${backendUrl}/api/patients/signup`, formDataObj);
...
<div className="min-h-screen pt-16 bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center p-4">
      <div className="bg-white p-10 shadow-2xl rounded-2xl w-full max-w-lg">
        <h1 className="text-3xl font-extrabold text-center text-blue-600 mb-6">Patient Sign Up</h1>
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data" className="space-y-4">
          ...
          {['Name', 'Age', 'Email', 'Phone', 'HistoryOfSurgery', 'HistoryOfIllness', 'Password'].map((field) => (
            <div key={field}>
              ...
              <input
                ...
                required={['Name', 'Age', 'Email', 'Phone', 'Password'].includes(field)}
              />
            </div>
          ))}
        ...
        <p className="mt-6 text-center">Already have an account? <Link to="/patient/signin" className="text-blue-500 hover:underline">Sign In</Link></p>
      </div>
    </div>
  );
};
